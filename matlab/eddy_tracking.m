...
for i = 3:length(f)
    ...
    tmp.eddy_center_lat{j}    = data.eddy_center_y;
    tmp.eddy_center_lon{j}    = data.eddy_center_x;
...
%     disp(['Timestep = ' num2str(it)])
    ...
    for i=1:i_cluster
        ...
        for i1=i1_start:total_tracks
            % in km
            dist = sqrt((lat_center{i1}(end )- curr_lat_center).^2 + (lon_center{i1}(end) - curr_lon_center).^2);
