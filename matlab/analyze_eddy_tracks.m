...
quiv = 0;
lw = 1;
min_steps = 2; % minimum track length to plot
...
for i = 1:length(edl)
    ...
    x = lon_center{j};
    y = lat_center{j};
    [y,x]=utm2ll(lon_center{j}*1000,lat_center{j}*1000,17);
    ...
    if quiv == 1
        ...
        larr = sqrt(xd^2 + yd^2);
        if direction{j}(1) > 0 
            m_quiver(xs,ys,xd,yd,0,'b','linewidth',lw,'maxheadsize',.2/larr)
        else
            m_quiver(xs,ys,xd,yd,0,'r','linewidth',lw,'maxheadsize',.2/larr)
        end

    else
        if w1 > 0
            m_plot(x,y,'-b.') % CCW (Northern: cyclonic/upwelling ; Southern, anticyclonic/downwelling)
        else
            m_plot(x,y,'-r.') % CW (Northern: anticyclonic/downwelling ; Southern, cyclonic/upwelling)
        end
    end
    m_plot(x(1),y(1),'o','LineWidth',2,'color',[0 .8 0])
    m_plot(x(end),y(end),'kx','LineWidth',2)
    if i == 1
