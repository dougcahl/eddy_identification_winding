...
npz_in_name = din_data + region + '_avg25hr_' + str(avg_25hr) + '_eddy_tracks_' + str(radar_km_resolution) + 'km.npz'
csv_out_name = din_data + region + '_avg25hr_' + str(avg_25hr)  + '_eddy_tracks_' + str(radar_km_resolution) + 'km.csv'
csv_out_name_junk = din_data + region + '_avg25hr_' + str(avg_25hr)  + '_eddy_tracks_' + str(radar_km_resolution) + 'km_junk.csv'
        ...
        for tracki in range(0,int(total_tracks)):
            ...
            if time1 in timetest: # if this time exists for this track
                ...
                if eddy_center_lon[tracki].size > 1: # eddy has a history or future from this timestep
                    # positions 
    #                print(eddy_center_lon[tracki][0:timei+1])
                    lons115 = np.array(eddy_center_lon[tracki][0:timei+1], dtype=np.float)
                    lats115 = np.array(eddy_center_lat[tracki][0:timei+1], dtype=np.float)
                    lons = np.around(lons115,5)
                    lats = np.around(lats115,5)
